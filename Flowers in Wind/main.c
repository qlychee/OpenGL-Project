...
static void drawPetals3(double x, double y, double z) {
...
static void drawPetals4(double x, double y, double z) {
...
static void drawCenter2(double x, double y, double z)
{
   //  Set specular color to white
   float white[] = {1,1,1,1};
   float Emission[]  = {0.0,0.0,0.01*emission,1.0};
...
void display()
{
  ...
  glLightfv(GL_LIGHT0, GL_POSITION, Position);
   ...
   Print("Z");
	...
	glWindowPos2i(5,25);
      Print("Wind Angle =%d Wind Speed = %0.f Flower Height = %0.f Number of Flowers = %d",
      thetaWind, speedWind,stemHeight,NUM_FLOWERS);
   glWindowPos2i(5,5);
...
void key(unsigned char ch, int x, int y)
{
   if (ch == 27)
      ...
      light = 1 - light;
	   ...
	   colorInd ++;
	   if(colorInd == 4){
		   	color[4].r=(double)rand()/(double)RAND_MAX;
			color[4].g=(double)rand()/(double)RAND_MAX;
			color[4].b=(double)rand()/(double)RAND_MAX;
		}
	   if(colorInd == 5){
		   colorInd =0; //reset
       }
      ...
      NUM_FLOWERS++;
   else if (ch == '{' && stemHeight >=0)
      stemHeight--;
   else if (ch == '}' && stemHeight <150)
      stemHeight++;
   else if (ch == '>') {
      thetaWind += 5;
