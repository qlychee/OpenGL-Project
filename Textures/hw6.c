...
void drawCylinder(double x, double y, double z, double scaleX, double scaleY, double scaleZ, double rotate) {
	...
	/* Side of cylinder */
    glBindTexture(GL_TEXTURE_2D,texture[2]); // wood
...
static void drawRoof(double x, double y, double z, double dx, double dy, double dz,double th){
  ...
  glEnable(GL_TEXTURE_2D);
  glTexEnvi(GL_TEXTURE_ENV,GL_TEXTURE_ENV_MODE,GL_MODULATE);
  glBindTexture(GL_TEXTURE_2D,texture[4]);//wood
...
static void drawHouse(double x, double y, double z, double dx, double dy, double dz,double th){
  ...
  // Body of house
  glBindTexture(GL_TEXTURE_2D,texture[0]); // stone wall
  glBegin(GL_QUADS);
  //Back
...
//Front
  ...
  glEnable(GL_TEXTURE_2D);
  glTexEnvi(GL_TEXTURE_ENV,GL_TEXTURE_ENV_MODE,GL_MODULATE);
  glBindTexture(GL_TEXTURE_2D,texture[1]);//metal
  glBegin(GL_TRIANGLES);
  // Front
  glNormal3f(0,0,+1);
  glTexCoord2f(0.0, 0.0); glVertex3f(+roofoff,+roofbot,+roofwid);
...
int main(int argc,char* argv[])
{
   ...
   //  Create the window
   glutCreateWindow("Elizabeth Qiu- HW6");
   ...
   texture[3] = LoadTexBMP("redBrick.bmp");
